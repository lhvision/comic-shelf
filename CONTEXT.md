...
- **WebMCP 最小特权角色矩阵（WebMCP Least-Privilege Role Matrix）**：浏览器端模型上下文协议在多身份场景下的权限划分规范。视口阅读与排版工具（跳页/翻页/分屏/气泡高亮）对全部角色（馆长/访客/单本直达）开放；书架检索只读工具对馆长与访客开放（自动应用隐私白名单）；而作品收录（import）、元数据编辑（update_metadata）与全本离线预缓存（cache_all）受前端 `canWrite` 哨兵与后端 `require_curator` 双轨拦截，严格限定为馆长专属。
