...
- **凭据边缘免检与安全级别跳过（Auth Cookie Edge Bypass & Security Level Skip）**：Cloudflare Anycast 边缘根据客户端携带的 `comic_shelf_token` 或 `comic_shelf_device` Cookie 实施的精细化质询豁免机制。在全域开启「Under Attack 模式」极高防扫描防护的前提下，自动跳过针对合法读者的安全级别（五秒盾）挑战，消除后台非交互式 API 与漫画原图加载因 `cf_clearance` 失效引发的 403 质询阻断。
