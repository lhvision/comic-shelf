...
- **API 优先双轨元数据解析（API-First Dual-Track Album Retrieval）**：第三方鉴权图源（如禁漫）的元数据解析双轨范式。优先通过注入 `AVS` 移动端凭据的官方 REST API 客户端（`_make_api_client`）直接拉取画卷详情，天然规避 Web 网页端针对受限画卷强制图形验证码（CAPTCHA）与 302 重定向至 `/error/album_missing` 的下架假象；遇网络异常或数据模型变动时无缝平滑降级至既有 HTML 抓取通道，兼具极速解析与高容灾韧性。
- **操作级 RPC 状态解耦与短时同文防抖（Operation-Level RPC Error Decoupling & Toast Debounce）**：前端单次瞬态交互（如收录作品、更新封面、重新装订）的错误隔离与通知呈现规范。操作级 RPC 失败严禁写入表征全库生命周期的全局 `store.error`，仅向外抛出异常由交互发起组件局部捕获并精准展示；全局 `useToast` 消息队列集成 1.5s 窗口同文同音调（text + tone）幂等去重防抖与计时器刷新，彻底消除并发重入与多链路冒泡导致的视觉重影与双重报错。
...
- **发现榜单与纯内存封面代理（Discovery Feed & In-Memory Cover Proxy）**：纸间聚合官方热门排行榜（禁漫 `jm`、哔咔 `picacg` 的日榜/周榜/月榜）的发现导流机制。榜单数据分源缓存至 `data/discovery/{source}_{timeframe}.json`；封面遵循**方案 A 按需触发**与**纯内存 LRU 代理（Scheme A In-Memory Proxy）**规范（零磁盘落盘），当榜单轮替淘汰作品时内存自然回收，绝对杜绝产生无主孤儿图片碎片。
...
- **封面序号自定义（Cover Indices）**：允许馆长在自建工坊或编辑资料弹窗中显式指定 4 个全局页码序号（如 `[1, 10, 25, 50]`），书架与详情页轮播卡片将按此顺序展示这 4 张页面作为封面。服务器路径导入时画页总数未知，选择器不封顶；越界页码写入后不会生效，读取封面时夹到 `1..page_count`。
...
- **画卷生命周期与自愈删除（Comic Lifecycle & Cascade Deletion）**：纸间完备的物理存储清理与数据对齐体系。包含：① 整本漫画彻底移除与全链路级联清理（物理清除画页原图、WebP 各级封面与缩略图目录；原子注销 SQLite `comics_index` 影子索引、`comic_dialogues_fts` 台词全文检索与 OCR 元数据、所有用户的 `user_reading_progress` 阅读进度与 `user_favorites` 喜欢标记，以及关联的 `direct_passes` 单本沙箱通行证；并在以图搜图等各检索端彻底排除幽灵记录）；② 单章节删除与全局页码自愈压实（移除指定话数后，全书剩余章节起始页与全局页码自动单调压实重排 $1 \dots N$，绝不留空洞或错页，自动刷新受影响封面）；③ 暂存区即时释放与 1 小时自动 TTL 保底；④ 重新装订覆盖式替换（免去手动去文件系统删文件）。
...
- **阅读状态分段筛选（Reading Status Segmented Filter）**：读者作品阅读进度的单选互斥维度（`全部` / `在读` / `已读完`），与正交的个人偏好（`只看喜欢`）解耦，由 `useShelfState` 内存响应式状态全局单例驱动，免除对 URL Query 的无谓污染与路由抢占，保障顶栏导航的原生丝滑切换。
...
- **多标签复合筛选（Multi-Tag Compound Filter）**：书架支持读者同时点选多个分类标签进行组合淘书。采用确定性交集（AND）收窄逻辑，即所选藏书必须同时包含所有已激活标签；配合溢出气泡抽屉支持连续点选且保持展开，状态由 `useShelfState` 内存响应式状态驱动，解耦并免除 URL 路由查询参数写入，保障跨页面导航顺畅无阻。
- **Hover 迟滞保护（Hover Hysteresis Guard / Zero-Jitter Hitbox）**：书架卡片在浮动位移（如 `translate: 0 -0.35rem`）时防止鼠标脱靶振荡的几何刚性保护机制。将 `:hover` 作用域锚定在位置静止的宿主容器，并在位移产生的下沿留白区挂载透明延伸命中判定区，彻底消除鼠标停留在卡片临界边缘时诱发的每秒数十次高速抖动死循环（Jitter Loop）。
...
- **台词语义召回（Dialogue Semantic Recall）**：按"意思相近"而非字面找台词的检索能力，与台词全文检索并列、结果互不混排；返回单个气泡及其语义相似度，不掺收藏、阅读进度这类本人信号。本阶段只面向 agent（MCP 与 HTTP 出口），书架不设入口（ADR 0028）。
- **台词相关度（Relevance / `rank_score`）**：关键词检索给每个命中页算的纯相关分，只在本次候选结果内部可比，换一次查询就换一把尺子；排序会在它之上叠加本人信号，但它自身永远不含这些信号。_避免_：跨查询比较，或与语义相似度相加。
- **语义相似度（Semantic Similarity / `similarity`）**：语义召回给出的分数，表示问题与一条台词意思有多近。只在同一次查询的结果之间比高低，不跨查询比较，也不当"命中/未命中"的阈值——"没找到"只由结果里的 `reason` 表达。与以图搜图的相似度百分比不是同一把尺子。_避免_：置信度、命中率。
- **简繁双向检索（Simplified/Traditional Bidirectional Search）**：消除海峡两岸汉化组字符差异的透明映射机制。输入简体自动覆盖港台繁体译文，输入繁体亦能命中大陆简体文本，杜绝精准匹配下的假性漏搜。**两个出口用的不是同一种补偿**：台词检索走**索引侧归一**——`comic_dialogues_fts.text_norm` 列在写入时就把整句折成简体字形，倒排只建在这一列上，查询侧只需 `to_simplified` 一次；`text` 列退为 UNINDEXED 的原文副本，展示与高亮仍吃页面上那套字形（`to_simplified` 是逐字符 1:1，偏移可直接回切）。书架书名/作者/标签搜索仍是**查询侧补偿**，走多组 `LIKE` OR（元数据里混用两套字形，规范不了写入侧；历史上书架这条漏做，真库实测「咲恋」只命中 1 本、「咲戀」命中 4 本，合起来 5 本谁都不全）。台词出口从查询侧改成索引侧的原因是实测：`zh_conv` 的简↔繁是逐字符一对一，一次查询最多炸出「原样 / 全简 / 全繁」三个**整串**写法，可汉化组语料里大量是**同一行内繁简混排**（字体缺字、嵌字只换了一半）。真库 1238 条含繁体字形的台词里有 **735 条（59%）用简体照着打出来就搜不到**——例：语料「縮紧」，旧写法拿「缩紧」和「縮緊」去撞，两个都不等于「縮紧」；索引侧把两侧都折成简体后必然汇合。
...
- **气泡呼吸高亮（Breathing Bubble Overlay）**：阅读器接收到台词检索直达意图时（`?page=42&bubble_box=...`，同页其余命中气泡由 `&bubble_boxes=a;b;c` 一并带过来），在目标画页对应的归一化气泡矩形（`[ymin, xmin, ymax, xmax]`）上浮现暖纸朱砂金色半透明覆盖层：代表气泡呼吸、同页其余命中气泡只做静态描边，呼吸闪烁 2 秒后自然平滑淡出并原地静默擦除 URL 查询参数；描边框数与后端 `MAX_PAGE_BOXES` 同值封顶（解析与序列化双侧各夹一次，防手改 URL 刷出满屏框）；代表气泡坐标畸形时改用第一个合法坐标，绝不让同页其余命中气泡跟着消失；非台词检索进入或翻离当前页时物理不挂载覆盖层，零 DOM 重排且不打扰后续连续阅读心流。
- **代表气泡（Representative Bubble）**：一个命中页里相关度最高的那个气泡；阅读器据它定位，并只在它上面呼吸高亮。_避免_：代表格（漫画里「格」指分格）。
- **同页其余命中气泡（Sibling Hit Bubbles）**：同一命中页里除代表气泡外、同样命中查询的气泡；阅读器只给它们静态描边，数量有上限。界面计数用「处」（如「3 处命中」）。_避免_：其余格、其余命中格。
...
- **台词伴生资产（Sidecar OCR Asset / `{index}.ocr.json`）**：画页的结构化文字与气泡坐标伴生数据。记录单页分镜气泡的归一化相对坐标 `[ymin, xmin, ymax, xmax]`、对白文本、语种（zh/ja）、识别置信度与页内阅读顺序（`order`，竖排页自右向左分栏、横排页自上而下分行）。`order` 与气泡身份 `id` 各自独立成字段，不靠数组下标隐式承载顺序。与原图平级存储于 `pages/` 目录，与原图二进制解耦。
...
- **气泡级文本聚类（Bubble-level Line Clustering）**：OCR 伴生数据处理流水线中的空间几何合并规范。将单张画页中散落的多行文本框基于空间几何邻近与排版流向聚类为完整气泡，提取覆盖完整对白的单一归一化包围盒，避免单行截断搜索与破碎高亮框。邻近判据之外有三道否决闸，缺一道就会把整页糊成一个气泡：横竖不同不许同组、笔画粗细（竖排比列宽/横排比行高）比超过 2 倍不许同组、合并后外接框占页面积超 20% 拒绝合并（面积上限必须按分量整体判，只看成对距离会被传递闭包绕过）。聚类只重分组不增删文本，验证时看"字数守恒 + 逐串对照"。
...
- **单本沙箱临时通行证（Single-Book Direct Pass）**：由外部智能体（MCP）或馆长为特定单部漫画签发的高熵临时阅读凭据（`direct:{source}:{source_id}`）。在满足外链分享（如群聊淘书、名对白推荐）免密阅读的同时，施加绝对物理沙箱隔离：访问全库书架与统计直接 403 阻断，跨本访问阻断，所有写操作一律拦截；并在画页二进制端点实施基于 `_uid + client_ip` 复合限流（180 页/分钟），杜绝直达链接被爬虫暴破滥用。支持基于篇幅自适应分配的**动态时效（Dynamic TTL）**（短篇默认 2 小时，多卷/长篇自适应升至 24 小时，上限 7 天）；通过**三重无残留清理**（每次发新票被动顺手清、服务冷启动全量清、画卷删除级联清）保持数据库零碎片膨胀。前端视图层严格贯彻“零误导入口（Zero False Affordance）”契约，物理剥离详情返回、顶栏书架导航与储存管理入口，品牌印章降级为静态展示，结合全局路由守卫（Router Navigation Guard）将读者牢牢锚定在单一作品中。
- **机器人群私双轨权限模型（Bot Dual-Track Gating Model）**：群聊机器人（如 QQ / 飞书 Bot）对接纸间数据中枢时确立的最小特权分流标准。群聊公开场景仅开放只读工具（以图搜图、台词全文检索与单本沙箱链接签发），彻底屏蔽作品入库与离线预缓存指令；私聊场景严格核验馆长专属账号白名单，仅对通过白名单校验的馆长本人私聊指令放行车号收录入库与全本预缓存触发，物理消除群友滥用或越权入侵。
...
- **台词相关度排序（Dialogue bm25 Ranking）**：`comic_dialogues_fts` 走 `MATCH` 的检索分支以 `ORDER BY -bm25(comic_dialogues_fts) DESC` 召回，先排序后 `LIMIT` 截断。bm25 返回负数且越相关越负，取负后统一成"越大越相关"——与 2 字分支同向，业务信号才加得起来；`bm25()` 入参必须写完整虚拟表名而非 `FROM` 别名。两条硬边界：3 字以上走 `MATCH` 拿 bm25；2 字查询**产不出任何 trigram token**（实测 `可爱*`、`"可爱"`、`"可爱"*` 一律 0 行，故「前缀改写救两字词」在本项目栈不成立），只能落到 `LIKE` 扫描并改按 `词频 ÷ 文本长度` 排序——这正是 bm25 中真正区分候选的那两项（IDF 对单次查询是常数，除掉不影响顺序）。要在两字词上拿到真正的倒排加速，唯一出路是另建 bigram/单字侧表，不是改写查询；且在语料量级很小时 bm25 绝对值可低至 1e-6 量级、候选间仅在小数点后第六位分化，任何业务加权归一化都只能在候选池内取 min/max，写死满分常数必然失效——且 min/max 必须取**未做身份过滤的候选行**极值，否则隐藏本占住端点时同一页在访客与馆长眼中分数不同。
- **台词业务信号融合排序（Dialogue Signal Fusion Ranking）**：相关度之上的第二层排序，专治"顶到页坑位上限时留谁"。两条检索路径先统一成"越大越相关"（`MATCH` 支取负 bm25、`LIKE` 支取词频密度），在**本次候选池内** min-max 归一成 `rank_score`（0..1，跨查询不可比），再加热度（`views`/`likes` 展示字符串经 `_parse_metric_count` 解析后取对数、在本次池内那些书之间 min-max；空串算"没有热度"而非 0）＋收藏＋完读率＋最近阅读（30 天线性衰减）。三条不变量：**信号只抬升不压低、合计封顶 0.5**（相关度差 0.5 以上翻不了盘，实测 `老师` 池内前 20 名相关度全部打平在 1.0，正是这层在决定截断）；**无信号即抬升为 0**，顺序自动退回纯相关度，不设分支；**只有顺序随身份变**，同一页的 `rank_score`/`bubble_count`/`text` 与访客或馆长身份无关（见错题本 #139「计数类字段不得按身份分档」）。`rank_score` 语义永久固定为纯相关度，不掺任何业务信号抬升。
...
- **台词语料类别（Dialogue Kind Classification）**：`sync_comic_dialogues` 入库那一刻对每条 OCR 文本的三分类。`dialogue` 为分镜台词，正常参与检索；`paratext` 为书名页、版权页、汉化组 credits 与作者后记等**副文本**，入库但不进台词检索（它们是作者意图密度最高的语料，取料出口与语义召回要用，绝不删除）；`noise`（纯拉丁/数字的短水印与页码）直接丢弃不入库。判据只用**页位 + 文本特征**：前 2 页、末 4 页（带"整页像后记"护栏，避免误杀画到尾页的剧情）、特征词（图源/翻校/嵌字/QQ群/pixiv/comic market…）与"无中日文字形且 ≤12 字符"。**尾页护栏里"整页像副文本"的证据只认特征词，不认噪声行**：水印与页码本来就单独不入库，而正文页上同样满是水印——把噪声当证据会让一行 `？？？`（被 OCR 认成 `888`）连累整页剧情从检索里消失（真库实测误杀 60 页 / 119 行）。刻意不用"文字外面有没有闭合气泡框"的几何轮廓判据——实测在 501 条语料上它能抓住全部脏数据，但要陪葬 41% 的真台词（气泡内常与整页白底漏成一片，且矩形旁白框同样是闭合轮廓，几何上分不开）。特征词表刻意不收「我是」「禁止」这类会出现在真台词里的宽词。
- **台词全文索引表（Comic Dialogues FTS / `comic_dialogues_fts`）**：在独立专库 `comic_dialogues.db` 中维护的 SQLite FTS5 虚拟全文检索表（基于 Trigram 分词）。将 `{index}.ocr.json` 中的对白文本以倒排索引建表，支持读者在书架搜索栏通过模糊台词快速定位目标漫画、具体页码与气泡坐标。除参与匹配的 `text` 外，其余列一律 `UNINDEXED`，含 `reading_order`（页内阅读顺序，供下游按 `(page_index, reading_order)` 还原整话台词流）与 `kind`（语料类别，检索侧固定只取 `dialogue`）。`kind` 必须排在列尾——`snippet(comic_dialogues_fts, 4, ...)` 的列号按位置写死，新列插中间会让高亮摘要串到别的字段。FTS5 虚拟表不支持 `ALTER TABLE ADD COLUMN`，加列只能在 `init_dialogue_db` 里 drop 重建并同步清空 `comic_ocr_sync_meta`，否则 mtime 增量判断会跳过重灌令检索永久返空；重建自带 `backfill_dialogue_index()` 就地回填，不需要人工再跑一次同步。
- **台词取料出口（Story Context Export / `get_story_context`）**：与人用检索并列的第二条出口，服务漫画分镜、动漫脚本、游戏对话等下游生成管线。按「作品 + 页码区间」取料，**入参不接受关键词**；顺序跟随剧情本身（`page_index` + `reading_order`）而非相关度；行数由 `budget_lines` 预算决定（硬天花板 400 行 / 200 页），撞预算必须回报 `truncated`。返回拆两份：`lines`（`line_id`、页码、序号、原始 `text`）交给模型，`boxes`（`line_id` → 四点坐标）交给渲染层，靠 `line_id` 关联——**展示态的 `<mark>` 与 `snippet` 永不进入物料**。已知限制：无 `speaker` 数据来源（不吐字段也不用空串占位）；`reading_order` 是稳定序号而非连号，噪声行不入库后同页会留空洞。经两条通道暴露，都只认馆长口令与 Machine Token（访客与单本沙箱通行证拿不到）：Server-Side MCP 工具 `get_story_context`，与 HTTP 端点 `GET /api/search/dialogue-context`。
...
- **前端模型上下文协议（WebMCP / Web Model Context Protocol）**：基于 W3C / Chrome 浏览器级 `document.modelContext.registerTool()` 规范与 VueUse 15 `useWebMCP` 建立的端侧 AI Agent 标准交互契约。网站向浏览器内置或扩展 Agent 显式暴露结构化 Tool Schema，使 Agent 通过 RPC 参数直接操作视口（如翻页、切换排版、淘书检索），彻底终结传统基于截图与脆弱 DOM 探查模拟点击的低效模式；与后端 FastAPI `/mcp/sse`（数据中枢）构成「操作面与数据面」端云协同双轨架构。
- **视口生命周期绑定工具（Viewport-Scoped Tools）**：前端 WebMCP 工具的可用性严格锚定于 Vue 组件挂载生命周期（`ScopeDispose`）。在书架（`LibraryView`）时仅暴露淘书检索与藏书直达工具；进入阅读器（`ReaderView`）自动切入翻页、跳页、排版模式与对白气泡聚焦工具；离开视口即刻自动注销，杜绝跨页面非法调用与上下文污染。
- **画卷意图直达（Canvas Intent Direct-Calling）**：读者或 AI Agent 无需在复杂界面中寻找控件或模拟手势，直接通过结构化意图指令（如 `reader_jump_to_page`、`reader_switch_mode`）直接驱动阅读器核心状态，实现零延迟、高保真的纯净读图微操。
- **Vapor 局部探针（Vapor Mode Local Probe）**：在全站维持成熟稳定 VDOM 架构的前提下，以单组件为原子粒度局部开启 Vapor 模式的工程试炼机制。通过叶子展示组件试点，在零侵入、零全站回归风险下验证无虚树运行时的吞吐量、GC 垃圾回收与内存开销。
...
- **纸间数据中枢（Paper Room Central Data Hub）**：纸间在多项目生态体系中的核心系统定位。作为全站漫画数据存储、重型特征检索（以图搜图 / 台词 FTS5）、单本沙箱临时凭据（`temp_token`）签发与视口微操核心；坚守“本地优先”与“纯净读者展馆”定位，对外暴露标准 REST API、Server-Side MCP（FastAPI `/mcp/sse` 与 stdio）及前端 WebMCP，充当多项目架构中的单一语义与事实真理源。
- **多项目独立仓库架构（Polyrepo Ecosystem Architecture）**：纸间生态确立的工程拓扑标准。主仓库 `comic-shelf` 维持绝对纯净与轻量，下游各个异构微服务与子项目（如飞书以图搜图 Bot `paper-feishu-bot`、AI 生图与矢量分层工坊 `paper-studio`、Galgame 语料清洗与微调管线 `paper-corpus`、2D 互动视觉小说引擎 `interactive-avg`）作为同级独立代码仓库组织，各自分立技术栈与依赖，仅通过标准 API、MCP 协议与伴生资产契约（Sidecar Assets）向纸间数据中枢交互。
- **伴生资产标准命名空间（Standard Sidecar Asset Namespace）**：纸间数据中枢为下游项目固化的扩展资产目录规范。在 `backend/data/library/<source>/<source_id>/pages/` 下收敛 `{index}.ocr.json`（分镜对白与气泡归一化坐标）、`{index}.layers.json`（VTracer 矢量分层手稿路径）、`{index}.panels.json`（单格分镜几何坐标）、`{index}.caption.txt`（多模态标注提示词），以及在 `backend/data/corpus/galgame/<game_id>.jsonl` 下收敛中日双语平行语料。
- **WebMCP 最小特权角色矩阵（WebMCP Least-Privilege Role Matrix）**：浏览器端模型上下文协议在多身份场景下的权限划分规范。全量写工具（作品收录 `import`、全本/单话离线预缓存 `cache_all/chapter`、单本直达凭证签发 `create_direct_pass`）受前端 `canWrite` 哨兵与后端 `require_curator` 双轨拦截，严格限定为馆长专属；视口阅读与排版工具（跳页/翻页/分屏/气泡高亮）以及书架检索对持证长期访客（Claimed Guest）开放（自动应用隐私白名单）；而**单本沙箱临时受访者（`isDirectPass`）实施绝对零协议暴露面（Zero MCP Attack Surface），全局关闭所有 WebMCP 工具注册**，彻底阻断外部 AI Agent 与脚本高频快速遍历抓取画页。
