...
- **台词语义召回（Dialogue Semantic Recall）**：按"意思相近"而非字面找台词的检索能力，与台词全文检索并列、结果互不混排；结果一页一条，取该页意思最近的那个气泡并给出语义相似度，不掺收藏、阅读进度这类本人信号。本阶段只面向 agent（MCP 与 HTTP 出口），书架不设入口（ADR 0028）。
- **台词相关度（Relevance / `rank_score`）**：关键词检索给每个命中页算的纯相关分，只在本次候选结果内部可比，换一次查询就换一把尺子；候选范围按查询者能看到的书划定，访客搜同一句拿到的分数可能与馆长有别；排序会在它之上叠加本人信号，但它自身永远不含这些信号。_避免_：跨查询比较，或与语义相似度相加。
...
- **简繁双向检索（Simplified/Traditional Bidirectional Search）**：无论输入简体、繁体，还是页面上繁简混排的写法，都能搜到同一段文字。台词检索在建索引时就统一字形（**索引侧归一**），书架的书名/作者/标签在查询时补偿（**查询侧补偿**）；两种做法的取舍与实测见 ADR 0017「决策 2 · 修订」。
...
- **页级聚合（Page-level Aggregation）**：台词检索按页交付结果：一页一行，同页多处命中合并为「N 处命中」，代表气泡是其中相关度最高的一个。检索的单位是气泡，展示的单位是页。
...
- **气泡呼吸高亮（Breathing Bubble Overlay）**：从台词检索直达阅读器时，代表气泡上浮现约 2 秒的朱砂金呼吸高亮，同页其余命中气泡只做静态描边，随后淡出并擦除直达参数；不是从检索进入、或翻离该页时不挂载。URL 参数、描边上限与坐标兜底规则见 DESIGN_NOTES。
...
- **台词伴生资产（Sidecar OCR Asset / `{index}.ocr.json`）**：与原图平级存放的 OCR 伴生文件，记录这一页每个气泡的位置、文本、语种、置信度与页内阅读顺序；它是台词的原件，台词索引都从它派生。阅读顺序与气泡身份是两个独立字段（ADR 0017 决策 6）。
...
- **气泡级文本聚类（Bubble-level Line Clustering）**：把 OCR 在同一气泡里识别出的多行文字合成一条台词和一个包围框；只重新分组，不增删文字。合并判据与防止整页糊成一团的否决闸见错题本 #142。
...
- **机器专属令牌与最小特权契约（Machine Token & Dedicated Machine Role）**：由环境变量 `COMIC_SHELF_MACHINE_TOKEN` 声明的内网机器调用凭据，在鉴权层被授予专有的 `"machine"` 角色。专司内网自动化流水线（如 Paper Studio 本地建卷、OCR 伴生台词同步）；未配置 MCP 子凭据时兼作 MCP 入口。与访客一样看不到对访客隐藏的书，严格阻断整本删除与通行证管理等高危管理员操作，实现凭证边界最小特权化。
- **MCP 子凭据（MCP Token / `COMIC_SHELF_MCP_TOKEN`）**：专门交给外部 agent 的凭据，只能打开 MCP；配置后机器专属令牌就不能再进 MCP，换掉它即可收回所有 agent 的访问权。只有设置了馆长密钥时才生效。
...
- **台词业务信号融合排序（Dialogue Signal Fusion Ranking）**：关键词结果的相关度相近时，用热度与本人的收藏、完读、近期阅读决定先后。这些信号只往前提、不往后压，翻不了相关度的盘；它们只改变顺序，不改变命中数与台词相关度（ADR 0017 决策 9；两字查询与 bm25 的边界见错题本 #146，身份无关口径见 #139）。
...
- **台词全文专库（Dialogue Dedicated Database / `comic_dialogues.db`）**：后端独立的 SQLite 数据库文件（`backend/data/comic_dialogues.db`），专门承载台词全文索引 `comic_dialogues_fts`、语义向量 `comic_dialogue_vectors` 与伴生同步元数据 `comic_ocr_sync_meta`。在万级与十万级（数百万至千万条台词）规模下，将高频耗时批量写入与用户端核心状态完全物理隔离，彻底消除写锁争抢与备份膨胀。
...
- **台词语料类别（Dialogue Kind Classification）**：入库时给每条 OCR 文本分的三类：**台词**（`dialogue`）参与检索；**副文本**（`paratext`：书名页、版权页、汉化信息、作者后记）保留在库里，但不进台词检索，也暂不建语义向量；**噪声**（水印、页码）不入库。分类只看页位与文本特征，判据与弃用几何轮廓的理由见 ADR 0017 决策 7、错题本 #141。
- **台词全文索引表（Comic Dialogues FTS / `comic_dialogues_fts`）**：台词专库里的全文索引，从台词伴生资产派生、可以随时整表重建——伴生文件才是原件，索引坏了重灌即可。检索只取台词类语料。表结构、加列即重建的口径与迁移坑见 ADR 0017 决策 1、错题本 #136/#145。
- **台词取料出口（Story Context Export / `get_story_context`）**：给下游生成管线用的第二条出口：按作品与页码区间、按剧情顺序导出原始台词，不收关键词，不带任何高亮等展示态，超出行数预算时明确标记截断；不输出说话人。形态与上限见 ADR 0017 决策 8，可用凭据见 DEPLOYMENT §9.5。
