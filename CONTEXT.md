...
- **WebMCP 最小特权角色矩阵（WebMCP Least-Privilege Role Matrix）**：浏览器端模型上下文协议在多身份场景下的权限划分规范。全量写工具（作品收录 `import`、元数据编辑 `update_metadata`、全本/单话离线预缓存 `cache_all/chapter`、单本直达凭证签发 `create_direct_pass`）受前端 `canWrite` 哨兵与后端 `require_curator` 双轨拦截，严格限定为馆长专属；视口阅读与排版工具（跳页/翻页/分屏/气泡高亮）以及书架检索对持证长期访客（Claimed Guest）开放（自动应用隐私白名单）；而**单本沙箱临时受访者（`isDirectPass`）实施绝对零协议暴露面（Zero MCP Attack Surface），全局关闭所有 WebMCP 工具注册**，彻底阻断外部 AI Agent 与脚本高频快速遍历抓取画页。
