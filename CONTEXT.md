...
- **气泡呼吸高亮（Breathing Bubble Overlay）**：阅读器接收到台词检索直达意图时（`?page=42&bubble_box=...`，同页其余命中格由 `&bubble_boxes=a;b;c` 一并带过来），在目标画页对应的归一化气泡矩形（`[ymin, xmin, ymax, xmax]`）上浮现暖纸朱砂金色半透明覆盖层：代表格呼吸、其余格只做静态描边，呼吸闪烁 2 秒后自然平滑淡出并原地静默擦除 URL 查询参数；描边框数与后端 `MAX_PAGE_BOXES` 同值封顶（解析与序列化双侧各夹一次，防手改 URL 刷出满屏框）；代表格坐标畸形时提升第一格合法坐标，绝不让同页其余格跟着消失；非台词检索进入或翻离当前页时物理不挂载覆盖层，零 DOM 重排且不打扰后续连续阅读心流。
