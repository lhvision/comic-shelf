...
- **画卷与章节展开软封顶（Stepped Unfold Cap / 120 顶线保护）**：全站长列表（首页书架、页面索引 `PageIndexGrid`、章节目录 `ChapterIndex`）在触发“展开全部”时的防卡死统一契约。当待展开条目超出 120 项（如 900+ 拆帧画页或数百科章节）时，系统实行 120 项软封顶保护，单次点击最多顺延 120 项，由尾格折叠卡与计数器如实提示未尽余量，杜绝海量 DOM 挂载与网络风暴导致的主线程冻结。
...
- **PDF 画卷解包与无损抽取（PDF Comic Unpacking & Lossless Stream Extraction）**：从 PDF 漫画中解包画卷的机制。优先通过底层图像对象无损直接导出内嵌 JPG/PNG/WebP 原始字节流，0 重编码、0 画质劣化、百页秒级落盘；若遇复杂图层拼版或矢量页面则自动降级为 300 DPI 高保真光栅化渲染。
- **双轨分话探测器（Dual-Track Chapter Detector）**：针对单体多话 PDF 资源的智能分话管道。第一轨优先解析 PDF 内置电子书签大纲（Bookmarks/TOC）；在无电子书签时自动切入第二轨，由本地轻量 OCR 引擎嗅探纸质印刷目录与各话扉页标志（如 `第 N 话`、`加笔/附录`），推断出结构化章节草案供馆长在自建工坊一键核验。
- **卷首独立章节（Front Matter Chapter / 卷首・彩页）**：单行本漫画中位于正文第 1 话之前的封面、彩插、版权说明与纸质目录页集合。独立作为首个章节收录（`Chapter(id="c0", title="卷首 / 目录")`），既完整留存图书原貌与目录鉴赏价值，又确保读者进入第 1 话时开卷即为正文分镜。
- **暂存分话确认面板（Staged Chapter Inspection & Splitter）**：自建工坊中接收到 PDF 后的交互视窗。展示系统自动探测出的话数、标题、起始页码与页数概要，允许馆长一键采纳或微调修正后秒级落盘入库。
- **多卷目录合集导入（Multi-Volume Directory Ingest）**：指定包含多个卷/话 PDF（如 `第01卷.pdf` ~ `第07卷.pdf`）的服务器目录一键批量入库。系统自动按照文件名自然序列进行拓扑排序，为每一卷生成独立的 `Chapter`，连续平滑串联全书页码并生成对应章节封面。
- **画卷生命周期与自愈删除（Comic Lifecycle & Self-Healing Deletion）**：纸间完备的物理存储清理与数据对齐体系。包含：① 整本漫画彻底删除（清除画页原图、WebP 各级封面与缩略图、注销 SQLite 索引与全文检索台词）；② 单章节删除与全局页码自愈压实（移除指定话数后，全书剩余章节起始页与全局页码自动单调压实重排 $1 \dots N$，绝不留空洞或错页，自动刷新受影响封面）；③ 暂存区即时释放与 1 小时自动 TTL 保底；④ 重新装订覆盖式替换（免去手动去文件系统删文件）。
...
- **快捷指令选单 / 快捷指令中枢（Slash Command Palette / Search Command Menu）**：书架搜索栏键入 `/` 呼出的纸间快捷指令选单（`/台词`、`/车号`、`/作者`、`/随机`）。支持键盘上下导航、Tab / Enter 快速补全与简写别名（`/d`、`/id`、`/a`、`/r`）；通过全局层叠上下文隔离与 `--z-dropdown` 标尺约束，浮层在卡片流与标签栏上方自适应展开，统一案头检索与模式切换。
...
- **框架长效缓存切片（Vue Core Immutable Chunking）**：构建层将全站四大底层核心运行时（`@vue/`, `vue`, `pinia`, `vue-router`, `@vueuse/core`）固化收敛至单一 `vue-core.js`（~53 kB gzip）的分包契约。使核心框架依赖与高频迭代的业务逻辑入口（`index.js`，~23 kB gzip）解耦，配合服务端 `Cache-Control: immutable` 实现月度与跨版本长效强缓存，彻底消除业务微调引发的 90+ kB 框架层全量缓存雪崩。
- **权限组件按需分流（Curator View Code-Splitting）**：前端视图层将门禁大门（`GateView.vue`）、通行证簿（`GuestModal.vue`）与作品收录面板（`ImportPanel.vue`）等低频或权限专用的重型组件转为 `defineAsyncComponent` 异步按需加载的架构规范。已登录读者与普通借阅访客首屏 0 字节冗余代码；结合 PWA Service Worker 维持的 981 KiB 全量预缓存预算，馆长点击交互时仍能毫秒级从端侧 Cache Storage 秒开，兼顾访客首屏纯净度与馆长端无感延迟。
