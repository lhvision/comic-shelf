...
| 规则文档                           | 适用任务场景                                                                                     |
| ---------------------------------- | ------------------------------------------------------------------------------------------------ |
| `CONTEXT.md`                       | 纸间领域模型与术语表（单一语义源，核心概念、收藏夹状态、阅读器与基础设施定义）                   |
...
| `docs/adr/`                        | 架构决策记录集（系统重大权衡与不可逆技术方案全景，ADR 0001 ~ 0027）                              |
...
15. **契约自解释与精准解构门禁（Self-Documenting Contracts & Precise Destructuring）**：新增或重构 Composable 及子组件时，必须编写完整 JSDoc/TSDoc 注释（注明文件职责、入参说明、返回值说明、组件 Props/Emits 业务契约）；视图消费层只解构当前模板或方法实际使用的 Ref 与函数，严禁无脑全量解构导致 TS6133 冗余声明或模板漏绑。全站已在 `tsconfig.app.json` 开启 `noUnusedLocals` / `noUnusedParameters` 并在 `vite.config.ts` 开启 `no-unused-vars` 双轨严格阻断。
