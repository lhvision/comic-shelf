...
| MCP Server + 官方 Skill                        | `backend/app/routers/mcp.py`（9 工具 / 3 资源 / 2 提示词）、`skills/paper-room/SKILL.md` | 握手（SSE）与 RPC 两条路由验凭据（`_require_mcp_auth`），续话端点只认握手时发的 `session_id`：认馆长口令与 **`COMIC_SHELF_MCP_TOKEN` 子凭据**（配了子凭据后机器密钥不再能开 MCP；子凭据只在配了 `COMIC_SHELF_SECRET` 时生效），访客与匿名进不来；工具内部取数仍是馆长级，但 `create_direct_pass` 硬拒绝为隐藏本签发（见议题 D）。工具与资源处理器在线程池里跑。SSE 自带 `no-cache, no-transform` + `X-Accel-Buffering: no` |
...
11. ✅ **REST 接口不再能无限猜馆长口令**：此前任何 `/api/*` 带错凭据只回 401、不计数，登录锁与 MCP 锁都能绕过。现在 `enforce_credential_attempts` 在中间件与事件流入口执行：凭据对不上任何身份（馆长口令、机器密钥、直达票据、访客证都不是）记一次失败，与 `/api/auth/login` **共用同一个 IP 计数键**；已锁定的 IP 带任何凭据都回 429，连正确口令也不例外。持有访客设备会话的人另带 token 猜口令同样计数。401 时顺手清掉失效的登录 Cookie，免得馆长改口令后旧浏览器一屏画页就把本 IP 锁住。
12. ✅ **MCP 鉴权成功不清零计数**（见第 8 条）。
13. ✅ **直达票据不能订阅事件流**：事件广播的是全库书号（含隐藏本），分享出去的链接此前能在有效期内持续旁听。
14. ✅ **机器密钥经取料接口读不到隐藏本**：`/api/search/dialogue-context` 此前没有隐藏过滤，是机器密钥唯一能读隐藏本台词的口子。
...
- ⏳ **第二次复审记下、有意延后的小项**（都不构成绕过，按需再做）：
  ...
  - 语义向量缓存戳是"行数 + 最大更新秒"，同一秒内重同步且行数不变时会读到旧矩阵（按代码推理，未复现）；
  - `mcp.py` 里原有的两处类型警告。
