# 纸间设计演进里程碑归档（Milestones §01 – §52）

> 📦 **文档性质**：历史演进归档（Archive）。
> 本文件完整收录「纸间 · Paper Room」自立项初期（§1–§4）至第 52 次架构重构与交互迭代的全部设计推演、挑刺与落地记录。
> **最新设计系统常青规范与核心红线**请查阅主文档：[`DESIGN_NOTES.md`](../../DESIGN_NOTES.md)。
...
## 45. PWA Prompt 模式装订更新与零轮询 SSE 单向事件流（ADR 0009）

- **背景与痛点**：
  1. **自动刷新打断沉浸阅读**：旧 PWA 采用 `autoUpdate` 自动激活模式，当读者正在沉浸翻阅长篇漫画时，后台检测到前端发版若自动刷新会强行打断阅读节律；
  2. **传统轮询造成服务器空耗**：若采用前端定时 `setInterval(fetch, 60s)` 轮询检测更新，不仅对低功耗 NAS 带来无谓的 CPU 与网络空载，且无法做到实时通知。
- **架构决策与设计落地（ADR 0009）**：
  1. **PWA Prompt 模式与轻量装订横幅（UpdateBanner.vue）**：
     - `vite.config.ts` 将 VitePWA 切换为 `registerType: 'prompt'`；
     - 前端封装 `usePwaUpdate.ts` 捕获 `needRefresh`，弹出水墨质感悬浮装订横幅（`UpdateBanner.vue`），包含「立即装订」与「稍后」；
     - **阅读器自适应避让**：在全屏沉浸阅读器路由（`route.name === 'reader'`）下，横幅自动隐退，绝不遮挡画卷；
     - **状态收敛闭环**：读者点击「稍后」时，提示收拢至顶栏 `<StoragePopover />` 的朱砂小红点，面板内提供「新卷本装订就绪」卡片可随时补订。
  2. **单向系统事件流（SSE `/api/events/stream` & events.py）**：
     - 后端基于 `asyncio.Queue` 内存队列构建轻量 SSE 事件流通道，空闲时 0 CPU 占用；
     - 服务端版本发布、藏书增删（`import_comic` / `delete_comic`）与后台缓存完成时即时向全网在线客户端广播事件；
     - 前端 `useSystemEvents.ts` 收到 `library_changed` 事件后自动静默更新书架 Store（`load(true)`），多端实现实时同步；
     - 采用带随机抖动的指数平滑退避算法进行断线重连，防止服务器重启并发惊群。
  3. **安全性与防护边界**：
...
- **架构决策与重构落地**：
  ...
  - 将 `api.detail` 元数据加载、`AbortController` 竞态取消、404/网络异常 Toast 容错重定向、`[source, sourceId]` 动态变更防御性重入、`lastRead` 本地进度同步全部收敛至 `src/composables/useReaderData.ts`；
  - 暴露 `onLoaded` 生命周期契约，由 `ReaderView.vue` 在数据成功就绪后进行物理定位与 HUD 编排，彻底解耦组合式函数间的闭包依赖与 TDZ 隐患；
  - 结合 `vueuse-functions` skill，采用 `useToggle` 管理设置面板状态、`usePreferredReducedMotion` 监听动效偏好；
  - `ReaderView.vue` 脚本严格控制在 ≤150 行内，彻底合规。
  ...
  3. **跨话悬浮横幅组件抽离与移动端对齐（`ReaderChapterBanners.vue`）**：
     - 抽取 `src/components/reader/ReaderChapterBanners.vue`，接管话首「上一话」与话末「本话完 · 下一话」导航交互胶囊与滚动驱动浮入动效；
     - 严格落实 `AGENTS.md` Rule 12 统一图标收敛，采用 `<IconArrowLeft>` 与 `<IconArrowRight>` 替代 Unicode 字符；
     - 解决移动端长标题折行畸变与动画 `transform` 叠加引发的横向偏位问题，统一 `translate: -50% 0` 居中物理定位与自适应打点截断；
...
- **验证**：
  - `vp check`（193 个文件 0 warning / 0 lint / 0 type error）；
  - `vp test` 精准单测覆盖 `ReaderComponents.spec.ts`、`useReaderData.spec.ts`、`ReaderAutoTurn.spec.ts`、`ReaderLoadingState.spec.ts`、`ReaderProgress.spec.ts`、`useReaderSettings.spec.ts` 全部通过（24/24 用例全绿）。
