...
- **本质**：
  ...
  2. **本地自建车号 LOC_loc\_ 双重前缀口吃（Local Display ID Prefix Stutter）**：本地上传未指定 ID 时，后端曾默认生成 `source_id = f"loc_{YYYYMMDD_HHMMSS}"`，随后在拼接 `display_id` 时再次拼装 `f"LOC_{source_id}"`，导致车号变成难看的 `LOC_loc_...` 叠字口吃，破坏封面图书馆借阅印章的简洁性。解决之道：`source_id` 不再内置 `loc_` 前缀；`LocalProvider.normalize_id()` 与 `display_id()` 剥离冗余 `loc_`，印章只加一层 `LOC_`。空车号分配见 ADR 0027（上传走时钟，路径导入走文件夹名），不要把「剥离前缀」理解成「一律时间戳」。
- **红线与防误伤**：
...
- **放行/改用**：
  ...
  2. **本地标识双轨**：网页上传 / 暂存 PDF 未填车号时用 `YYYYMMDD_HHMMSS`；服务器路径导入未填车号时用文件夹名或 PDF 文件名（sanitize 失败才退回时钟），印章统一 `LOC_{source_id}`。不要把「一律时间戳」写回术语表。
...
- **红线与防误伤**：
  ...
  - **归一化字段的标尺是保密例外，不按统计口径处理**（2026-09-22 起标尺取未过滤候选行，2026-09-24 按安全审查改回）：`rank_score` 是池内 min-max。若标尺连隐藏本的行一起取，访客看到的最高分一旦低于 1.0，就等于告诉他"某本对你隐藏的书里有更贴切的这句话"（实测插一条隐藏命中，可见页从 1.0 掉到 0.424）。**保密优先于"数值与身份无关"**：标尺只取调用者看得见的候选行（`visible_scores`），同一页在访客与馆长下分数可以不同——它本就只在本次候选结果里可比。`bubble_count` 等计数仍守上一条；
  - 仍未堵上的旁路：隐藏行会占 200 行候选池的名额，访客在高频句上拿到的结果可能少于可见命中总数。只透露"变少了"、不透露分数，暂记待办（见 `docs/HANDOVER_NEXT_STAGE.md`）；
  - 改动这两处时，`backend/tests/test_dialogue_fts.py` 里两条用例（脏 id 写删往返、`test_dialogue_rank_score_scale_hides_hidden_books`）会红，不要把断言改回旧行为；
