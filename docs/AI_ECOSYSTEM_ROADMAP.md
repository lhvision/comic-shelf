...
- `detail_create_direct_pass({ page?, ttl_seconds? })`：
  为当前漫画签发单本沙箱临时直达阅读通行证（携带独立 `temp_token`，默认有效时长 2 小时，受访者仅能阅读当前漫画且无法访问书架其他私密典藏与写接口）。**【仅馆长权限】**
...
| **偏好与红心标记**       | `reader_toggle_favorite`<br>`detail_toggle_favorite`                                                                                                                                                                                                                                                            |     ✅ 允许     |  ✅ 允许（本地/借阅账单）   |    ❌ 零暴露（不注册进上下文）    | 访客标记喜欢保存在客户端 IndexedDB 离线账本；临时沙箱用户不开放标记工具。                                                                                  |
...
| **单本直达通行证签发**   | `detail_create_direct_pass`                                                                                                                                                                                                                                                                                     |     ✅ 允许     | ❌ 零暴露（不注册进上下文） |    ❌ 零暴露（不注册进上下文）    | 前端 `canWrite` 哨兵在注册层直接掐断，后端无条件执行 `require_curator` 强校验，严禁二次转签。                                                              |
...
### 4. 台词检索的学习式评判/重排模型（暂缓，记录启用前提）

- **做法（2026-09 记录的参考方案）**：小型编码器（如 `xlm-roberta-base`）接一个 `Linear(768, 1)` 打分头，多任务联合训练——成对偏好（RLCD 式 `-log σ(r₁ - r₂)`）、二分类判断（BCE）、多选一（CE）、打分分布（软标签交叉熵）四项损失相加，用来判断"这句台词是否回答了这个查询"并给候选重排。
- **可能的落点**：替代或补充关键词出口的 `rank_score` 与语义出口的 `similarity`（两者目前都只在单次查询内可比，没有可用的绝对阈值，见 ADR 0028）。
- **启用前提（缺一不做）**：
  1. **先有评测集与标注数据**：成规模的"查询 → 应命中页"标注与成对偏好样本；没有评测集就无法证明它比现行排序好；
  2. **满足延迟预算**：书架检索要保持毫秒级，重排只能作用在已截断的少量候选上，且需实测 CPU 推理耗时；
  3. **出现人类用户**：现阶段语义出口只面向 agent（ADR 0028），排序质量的收益要等书架上有人用才兑现。
- 本条只是记录，**代码不动**。
