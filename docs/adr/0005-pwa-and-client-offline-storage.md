...
3. **双粒度测量与安全清理交互**：
   ...
   - 细分展示「纸间核心资产」（直接度量 `workbox-precache-*` 桶，~1 MB）与「漫画阅览缓存」（基于浏览器真实物理配额扣除核心外壳反求得出，杜绝抽样偏差与核心资产虚假膨胀）；
   - 清理操作提供两档选择：
     - **日常级**：「清理阅览图片缓存」——只清空图片与插画 Cache 桶，保留 App Shell 核心外壳，应用断网依然可启动；
...
3. **端侧元数据持久化与离线模式韧性架构（Client Metadata IndexedDB Cache & Offline Mode）**：
   ...
   - **纸室离线模式与无缝容错**：断网自适应激活静默离线接管，书架全景陈列；详情页与阅读器断网时不强退首页，未缓存画页以降级水墨纸印骨架占位呈现；
