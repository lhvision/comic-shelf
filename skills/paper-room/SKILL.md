...
- **单本/批量收录**：调用 WebMCP `shelf_import_comic({ source: 'jm', id: ... })` 或 `shelf_batch_import_comics({ source: 'jm', items: "JM111111, JM222222" })`（`source` 必填，单批次严格同源以杜绝跨平台车号冲突；上限 50 本，内部 5s 安全间隔串行防风控）；
